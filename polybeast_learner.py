...
def inference(
    inference_batcher, model, flags, actor_device, optimizer, lock=threading.Lock()
):  # noqa: B008
    with torch.no_grad():
        for batch in inference_batcher:
            ...
            #print("pg inference prev_action", observation['inv_oclasses'])
            ...
            step = 0
            #print('pg opt state dict', list(optimizer.state_dict()['state'].keys()))
            #if len(list(optimizer.state_dict()['state'].keys())) > 0:
            #    step = optimizer.state_dict()['state'][2]['step']
            #print('pg step', step)
            with lock:
                outputs = model(observation, agent_state, step)
...
def learn(
    ...
    for tensors in learner_queue:
        ...
        #print('pg learner info', info)

        #print('pg prev_action', prev_action)

        #print("pg actor outputs", actor_outputs[0].shape)

        lock.acquire()  # Only one thread learning at a time.

        #print("pg inside learn, step:", [optimizer.state_dict()['state'][x]['step'] for x in list(optimizer.state_dict()['state'].keys())])
        step = 0
        #if len(list(optimizer.state_dict()['state'].keys())) > 0:
        #    step = optimizer.state_dict()['state'][2]['step']
        #print('pg step', step)
        output, _ = model(observation, initial_agent_state, step, learning=True)
...
def train(flags):
    ...
    inference_threads = [
        threading.Thread(
            ...
            args=(inference_batcher, actor_model, flags, actor_device, optimizer),
        )
